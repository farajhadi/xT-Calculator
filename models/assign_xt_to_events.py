...
class xTAssigner:
    ...
    def analyze_top_actions(self, events_df, n=10):
        
        actions = events_df[
            (events_df['event_type'].isin(['Pass', 'Carry', 'Shot', 'Dribble'])) & 
            (events_df['xT_delta'].notna())
        ].copy()
        
        print("\n" + "="*80)
        print(f"TOP {n} xT-GENERATING ACTIONS")
        print("="*80)
        
        top_actions = actions.nlargest(n, 'xT_delta')
        
        for idx, (_, action) in enumerate(top_actions.iterrows(), 1):
            player = action['player'] if pd.notna(action['player']) else 'Unknown'
            team = action['team']
            event_type = action['event_type']
            
            if event_type == 'Shot':
                start_zone = int(action['start_zone'])
                start_loc = self.grid.get_zone_center(start_zone)
                
                xT_start = action['xT_start']
                xG = action['xT_end']  # For shots, xT_end is xG
                xT_delta = action['xT_delta']
                
                print(f"\n{idx}. {player} ({team}) - {event_type}")
                print(f"   Zone {start_zone} {start_loc}")
                print(f"   xT: {xT_start:.4f} → xG: {xG:.4f} (Δ = +{xT_delta:.4f})")
            else:
                start_zone = int(action['start_zone'])
                end_zone = int(action['end_zone']) if pd.notna(action.get('end_zone')) else start_zone
                start_loc = self.grid.get_zone_center(start_zone)
                end_loc = self.grid.get_zone_center(end_zone)
                
                xT_start = action['xT_start']
                xT_end = action['xT_end']
                xT_delta = action['xT_delta']
                
                print(f"\n{idx}. {player} ({team}) - {event_type}")
                print(f"   Zone {start_zone} {start_loc} → Zone {end_zone} {end_loc}")
                print(f"   xT: {xT_start:.4f} → {xT_end:.4f} (Δ = +{xT_delta:.4f})")
    
    
    def analyze_worst_actions(self, events_df, n=10):
        ...
        print("\n" + "="*80)
        print(f"WORST {n} xT-LOSING ACTIONS (Negative Value)")
        print("="*80)
        
        worst_actions = actions.nsmallest(n, 'xT_delta')
        
        for idx, (_, action) in enumerate(worst_actions.iterrows(), 1):
            player = action['player'] if pd.notna(action['player']) else 'Unknown'
            team = action['team']
            event_type = action['event_type']
    ...
    def player_summary(self, events_df):
        ...
        player_stats = actions.groupby(['player', 'team']).agg({
            'xT_delta': ['sum', 'mean', 'count'],
            'event_type': lambda x: (x == 'Pass').sum()
        }).reset_index()
        
        player_stats.columns = ['player', 'team', 'total_xT', 'avg_xT', 'actions', 'passes']
        
        for p_idx, player_row in player_stats.iterrows():
            player_actions = actions[actions['player'] == player_row['player']]
            player_stats.loc[p_idx, 'carries'] = (player_actions['event_type'] == 'Carry').sum()
            player_stats.loc[p_idx, 'shots'] = (player_actions['event_type'] == 'Shot').sum()
            player_stats.loc[p_idx, 'dribbles'] = (player_actions['event_type'] == 'Dribble').sum()
        
        player_stats = player_stats.sort_values('total_xT', ascending=False)
        
        print("\n" + "="*80)
        print("PLAYER xT CONTRIBUTIONS (Top 10)")
        print("="*80)
        print(f"\n{'Player':<25} {'Team':<20} {'Total xT':>10} {'Avg xT':>10} {'Actions':>8}")
        print("-"*80)
        
        for _, player in player_stats.head(10).iterrows():
            print(f"{player['player']:<25} {player['team']:<20} "
                  f"{player['total_xT']:>10.4f} {player['avg_xT']:>10.4f} "
                  f"{int(player['actions']):>8}")
